...
public class RegisterProduct extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Product product = productFacade.getByProductName(productName);
        
        try (PrintWriter out = response.getWriter()) {
            if(product != null){
                request.getRequestDispatcher("registerProduct.jsp").include(request, response);
                out.println("<br><br><br>Product already exists!");
            } else{
                Product x = new Product(productName,price,manufacturer,quantity,"active".charAt(0));
                productFacade.create(x);
